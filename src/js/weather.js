'use strict';
/* Japan Travel Guide — weather.js
   Open-Meteo only (forecast + geocode + air quality). NWS paths disabled.
   UI/structure ported from USA guide; Japan city majors.
   Load order: data/i18n → settings → i18n → weather.js → nav → nav-return → app.js
...
(function () {
  ...
  // ── Japan Travel Guide runtime bridge (replaces USA core/runtime globals) ──
  function jtgLang() {
    try {
      var L = (window.JTG && JTG.Settings && JTG.Settings.get("lang")) || "en";
      if (L === "zh-CN") return "zh";
      return L === "ja" || L === "zh" || L === "en" ? L : "en";
    } catch (e) { return "en"; }
  }
  var currentLang = jtgLang();
  var currentTempUnit = "c";
  var currentDistUnit = "km";
  try {
    if (window.JTG && JTG.Settings) {
      currentTempUnit = JTG.Settings.get("tempUnit") === "f" ? "f" : "c";
      currentDistUnit = JTG.Settings.get("distanceUnit") === "mi" ? "mi" : "km";
    }
  } catch (e) {}
  function getI18nDict(lang) {
    try {
      if (!window.JTG || !JTG.TRANSLATIONS) return null;
      var L = lang === "zh" ? "zh-CN" : lang;
      if (!L || L === "en") return null;
      var nest = JTG.TRANSLATIONS[L];
      if (!nest) return null;
      // Flatten nested weather.* and tools.* for USA-style flat keys
      var flat = {};
  ...
  window.addEventListener("jtg:settings", function () {
    try {
      if (window.JTG && JTG.Settings) {
        currentTempUnit = JTG.Settings.get("tempUnit") === "f" ? "f" : "c";
        currentDistUnit = JTG.Settings.get("distanceUnit") === "mi" ? "mi" : "km";
  ...
  const WIND_KEY = 'jtg-weather-wind';
  const PRECIP_KEY = 'jtg-weather-precip';
  const PRESS_KEY = 'jtg-weather-pressure';
  const FAV_KEY = 'jtg-weather-favorites';
  const MYLOC_KEY = 'jtg-weather-myloc';

  const MAJOR = [
    { name: 'Tokyo', admin1: 'Tokyo', lat: 35.6762, lon: 139.6503, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Yokohama', admin1: 'Kanagawa', lat: 35.4437, lon: 139.6380, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Osaka', admin1: 'Osaka', lat: 34.6937, lon: 135.5023, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Nagoya', admin1: 'Aichi', lat: 35.1815, lon: 136.9066, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Sapporo', admin1: 'Hokkaido', lat: 43.0618, lon: 141.3545, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Fukuoka', admin1: 'Fukuoka', lat: 33.5904, lon: 130.4017, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kobe', admin1: 'Hyogo', lat: 34.6901, lon: 135.1956, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kyoto', admin1: 'Kyoto', lat: 35.0116, lon: 135.7681, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kawasaki', admin1: 'Kanagawa', lat: 35.5309, lon: 139.7030, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Saitama', admin1: 'Saitama', lat: 35.8617, lon: 139.6455, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Hiroshima', admin1: 'Hiroshima', lat: 34.3853, lon: 132.4553, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Sendai', admin1: 'Miyagi', lat: 38.2682, lon: 140.8694, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Chiba', admin1: 'Chiba', lat: 35.6074, lon: 140.1065, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kitakyushu', admin1: 'Fukuoka', lat: 33.8834, lon: 130.8752, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Sakai', admin1: 'Osaka', lat: 34.5733, lon: 135.4831, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Niigata', admin1: 'Niigata', lat: 37.9161, lon: 139.0364, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Hamamatsu', admin1: 'Shizuoka', lat: 34.7108, lon: 137.7261, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kumamoto', admin1: 'Kumamoto', lat: 32.8031, lon: 130.7079, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Sagamihara', admin1: 'Kanagawa', lat: 35.5715, lon: 139.3732, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Okayama', admin1: 'Okayama', lat: 34.6551, lon: 133.9195, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Kanazawa', admin1: 'Ishikawa', lat: 36.5613, lon: 136.6562, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Nagasaki', admin1: 'Nagasaki', lat: 32.7503, lon: 129.8777, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Naha', admin1: 'Okinawa', lat: 26.2124, lon: 127.6809, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Nara', admin1: 'Nara', lat: 34.6851, lon: 135.8048, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Hakone', admin1: 'Kanagawa', lat: 35.2324, lon: 139.1069, tz: 'Asia/Tokyo', country: 'Japan' },
    { name: 'Nikko', admin1: 'Tochigi', lat: 36.7199, lon: 139.6982, tz: 'Asia/Tokyo', country: 'Japan' },
  ];
   ...
   */
  const CITY_NAMES = {
    '35.676,139.650': { en: 'Tokyo', es: 'Tokyo', zh: '东京', ja: '東京' },
    '35.444,139.638': { en: 'Yokohama', es: 'Yokohama', zh: '横滨', ja: '横浜' },
    '34.694,135.502': { en: 'Osaka', es: 'Osaka', zh: '大阪', ja: '大阪' },
    '35.181,136.907': { en: 'Nagoya', es: 'Nagoya', zh: '名古屋', ja: '名古屋' },
    '43.062,141.355': { en: 'Sapporo', es: 'Sapporo', zh: '札幌', ja: '札幌' },
    '33.590,130.402': { en: 'Fukuoka', es: 'Fukuoka', zh: '福冈', ja: '福岡' },
    '34.690,135.196': { en: 'Kobe', es: 'Kobe', zh: '神户', ja: '神戸' },
    '35.012,135.768': { en: 'Kyoto', es: 'Kyoto', zh: '京都', ja: '京都' },
    '35.531,139.703': { en: 'Kawasaki', es: 'Kawasaki', zh: '川崎', ja: '川崎' },
    '35.862,139.645': { en: 'Saitama', es: 'Saitama', zh: '埼玉', ja: 'さいたま' },
    '34.385,132.455': { en: 'Hiroshima', es: 'Hiroshima', zh: '广岛', ja: '広島' },
    '38.268,140.869': { en: 'Sendai', es: 'Sendai', zh: '仙台', ja: '仙台' },
    '35.607,140.107': { en: 'Chiba', es: 'Chiba', zh: '千叶', ja: '千葉' },
    '33.883,130.875': { en: 'Kitakyushu', es: 'Kitakyushu', zh: '北九州', ja: '北九州' },
    '34.573,135.483': { en: 'Sakai', es: 'Sakai', zh: '堺', ja: '堺' },
    '37.916,139.036': { en: 'Niigata', es: 'Niigata', zh: '新潟', ja: '新潟' },
    '34.711,137.726': { en: 'Hamamatsu', es: 'Hamamatsu', zh: '滨松', ja: '浜松' },
    '32.803,130.708': { en: 'Kumamoto', es: 'Kumamoto', zh: '熊本', ja: '熊本' },
    '35.572,139.373': { en: 'Sagamihara', es: 'Sagamihara', zh: '相模原', ja: '相模原' },
    '34.655,133.919': { en: 'Okayama', es: 'Okayama', zh: '冈山', ja: '岡山' },
    '36.561,136.656': { en: 'Kanazawa', es: 'Kanazawa', zh: '金泽', ja: '金沢' },
    '32.750,129.878': { en: 'Nagasaki', es: 'Nagasaki', zh: '长崎', ja: '長崎' },
    '26.212,127.681': { en: 'Naha', es: 'Naha', zh: '那霸', ja: '那覇' },
    '34.685,135.805': { en: 'Nara', es: 'Nara', zh: '奈良', ja: '奈良' },
    '35.232,139.107': { en: 'Hakone', es: 'Hakone', zh: '箱根', ja: '箱根' },
    '36.720,139.698': { en: 'Nikko', es: 'Nikko', zh: '日光', ja: '日光' },
  };
  ...
  const NWS_POINTS_LS = 'jtg-nws-points-v1-unused';
  ...
  function alertsBlockHtml(alerts) {
    ...
    const cards = alerts.map(function (a) {
      ...
      bodyParts.push('<p class="weather-alert-source">' +
        escapeHtml(t('weather.alertSource', 'Open-Meteo')) +
  ...
  function countryLabelJP() {
    const L = lang();
    if (L === 'ja') return '日本';
    if (L === 'zh') return '日本';
    return 'Japan';
  }
  function countryLabelUS() { return countryLabelJP(); }
  ...
  function loadNameCacheFromSession() {
    ...
    ['es', 'zh', 'ja', 'en'].forEach((L) => {
      try {
        const raw = sessionStorage.getItem('jtg-wx-names-' + L);
        if (!raw) return;
