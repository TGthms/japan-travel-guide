#!/usr/bin/env python3
"""
Japan Travel Guide — Gallery Manager
==================================
...
def html_item_block(
    *,
    ...
    if video_filename:
        ...
        video_attr = f' data-video="assets/gallery/videos/{v_esc}"'
    badge = ""
    if video_filename:
        badge = (
...
def insert_gallery_html(block: str) -> None:
    """Append a gallery-item block inside #gallery-grid (before its closing tag).
    ...
    if not m:
        raise RuntimeError(
            "Could not find #gallery-grid in gallery.html — structure unexpected. "
...
def insert_i18n(slug: str, caption: str) -> int:
    """
    Add flat key gallery.item.{slug}.caption into ja / zh-CN language blocks.
    English caption is the HTML text. Flat keys work with JTG i18n flat-fallback.
    """
    ...
    def inject_lang_block(src: str, lang: str) -> str:
        ...
        # insert after language opening brace
        inserted += 1
        # place after first { of language object
        return src[: start + 1] + "\n" + line + src[start + 1 :]
...
def list_photos() -> list[dict]:
    ...
    for m in re.finditer(
        ...
        medium = attr(img_attrs, "data-medium") or (
            f"assets/gallery/medium/{Path(full).name}" if full else ""
        )
    ...
    if patch_html and GALLERY_HTML.is_file():
        ...
        def patch_item(match: re.Match) -> str:
            ...
            if "data-thumb=" not in block:
                block = re.sub(
                    r'(src="assets/gallery/thumbs/[^"]+")',
                    rf'\1 data-thumb="assets/gallery/thumbs/{html.escape(fname, quote=True)}"',
                    block,
                    count=1,
                )
            if "data-medium=" not in block:
                block = re.sub(
                    r'(data-(?:thumb|full)="[^"]+")',
                    rf'\1 data-medium="assets/gallery/medium/{html.escape(fname, quote=True)}"',
                    block,
                    count=1,
                )
            ...
            block = re.sub(
                r'data-medium="[^"]*"',
                f'data-medium="assets/gallery/medium/{html.escape(fname, quote=True)}"',
                block,
                count=1,
            )
            html_patched += 1
...
<title>Gallery Manager — Japan Travel Guide</title>
<style>
  :root {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add or remove photos in the Japan Travel Guide gallery"
    )
    parser.add_argument(
        "--cli",
        metavar="FOLDER",
